...
export default function DistributionReportPage() {
  ...
  const formatNumber = (val: any) => (val !== null && val !== undefined ? Number(val).toLocaleString() : "-");
  ...
  const fetchData = async (start: string, end: string) => {
    ...
    try {
      ...
      if (isCentreUser) userFilteredData = mappedData.filter((d: any) => d.centre.toLowerCase() === userCentre.toLowerCase());
      if (isZoneUser) userFilteredData = mappedData.filter((d: any) => d.zone.toLowerCase() === userZone.toLowerCase());
  ...
  const handleFilter = () => {
    const filtered = data.filter((item) => {
      ...
      const matchCentre = isCentreUser ? true : centre ? item.centre === centre : true;
    ...
    setFilterApplied(!!startDate || !!endDate || !!course || !!description || !!centre || !!zone);
    setCurrentPage(1);
  };
  ...
  const saveApportionment = async () => {
    try {
      ...
      if (res.status === 200) {
        ...
        Swal.fire({ title: "Success!", text: "Apportionment saved successfully!", icon: "success" });
      } else {
        Swal.fire({ title: "Failed!", text: "Failed to save apportionment.", icon: "error" });
      }
    } catch (err) {
      console.error("Error saving apportionment:", err);
      Swal.fire({ title: "Error!", text: "Error saving apportionment. Check console for details.", icon: "error" });
    }
  };
  ...
  handleFilter();
      ...
      <div className="flex justify-between items-center mt-4">
        ...
        <div className="flex gap-2">
          ...
          <Button className="bg-green-500" onClick={exportExcel}>Export Excel</Button>
