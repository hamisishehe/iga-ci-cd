...
export default function CentresPage() {
  ...
  const fetchDepartment = async () => {
    try {
      ...
      toast("Error loading department");
    }
  };

  // Filters + Pagination
  const filteredDepartment = department.filter((u) => {
  ...
  const paginatedUsers = filteredDepartment.slice(startIndex, startIndex + rowsPerPage);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex justify-between items-center mt-4">
        <div className="text-sm text-gray-600">
          Showing {startIndex + 1}–{Math.min(startIndex + rowsPerPage, filteredDepartment.length)} of {filteredDepartment.length} users
        </div>
